        ...
        <div class="gi-card">
            ...
            <div class="gi-resumen">
                <div class="gi-res-item">
                    ...
                    <div class="gi-res-value" style="color:{{ $inusuales > 0 ? '#ef4444' : '#22c55e' }};">
                        {{ $inusuales }}</div>
                </div>
                <div class="gi-res-item">
                    <div class="gi-res-label">📈 Elevados</div>
                    <div class="gi-res-value" style="color:#fbbf24;">{{ $elevados }}</div>
                </div>
                <div class="gi-res-item">
                    <div class="gi-res-label">✓ Normales</div>
                    <div class="gi-res-value" style="color:#22c55e;">{{ $normales }}</div>
                </div>
                <div class="gi-res-item">
                    <div class="gi-res-label">📉 Bajos</div>
                    <div class="gi-res-value" style="color:#60a5fa;">{{ $bajos }}</div>
                </div>
            </div>
        </div>
            ...
            <div class="gi-card">
                ...
                <div class="gi-grid">
                    @foreach ($datos as $cat)
                        ...
                        <div class="gi-cat-card {{ $cat['estado'] }}">
                            <div class="gi-cat-header">
                                <div class="gi-cat-left">
                                    <span class="gi-cat-icono">{{ $cat['icono'] }}</span>
                                    <span class="gi-cat-nombre">{{ $cat['categoria'] }}</span>
                                </div>
                                <span class="gi-badge {{ $badgeClass }}">{{ $badgeLabel }}</span>
                            </div>
                            ...
                            <div class="gi-montos">
                                <div class="gi-monto-item">
                                    ...
                                    <span class="gi-monto-value"
                                        style="color:{{ $cat['estado'] === 'inusual' ? '#ef4444' : 'var(--w-text)' }};">
                                        S/ {{ number_format($cat['gastoActual'], 2) }}
                                    </span>
                                </div>
                                <div class="gi-monto-item">
                                    <span class="gi-monto-label">Promedio</span>
                                    <span class="gi-monto-value" style="color:var(--w-muted);">
                                        S/ {{ number_format($cat['promedio'], 2) }}
                                    </span>
                                </div>
                                @if ($cat['diffPct'] != 0)
                                    <div class="gi-monto-item" style="margin-left:auto;">
                                        <span class="gi-monto-label">Variación</span>
                                        <span class="gi-monto-value" style="color:{{ $diffColor }};">
                                            {{ $cat['diffPct'] > 0 ? '+' : '' }}{{ $cat['diffPct'] }}%
                                        </span>
                                    </div>
                                @endif
                            </div>
                            ...
                            <div class="gi-sparkline">
                                @foreach ($cat['historico'] as $i => $h)
                                    ...
                                    <div class="gi-spark-bar"
                                        style="height:{{ max(4, $altura) }}%; background:{{ $color }};"
                                        title="{{ $h['mes'] }}: S/ {{ number_format($h['gasto'], 2) }}">
                                    </div>
                                @endforeach
                            </div>
                            <div
                                style="display:flex; justify-content:space-between; font-size:0.6rem; color:var(--w-muted); margin-top:0.25rem;">
                                <span>{{ $cat['historico'][0]['mes'] }}</span>
                                <span>Este mes</span>
                            </div>
                        </div>
                    @endforeach
