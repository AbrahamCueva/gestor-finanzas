<x-filament-panels::page>
    ...
    <div class="rm-space">
        ...
        <div class="rm-selector-bar">
            ...
            <span class="rm-period-label">{{ ucfirst($datos['nombreMes']) }}</span>
        </div>
        <div class="rm-grid-3">
            ...
            <div class="rm-card">
                ...
                <div class="rm-kpi-value" style="color:#22c55e;">S/ {{ number_format($datos['totalIngresos'], 2) }}
                </div>
                <div class="rm-kpi-sub">{{ $datos['ingresos']->count() }} transacciones</div>
            </div>
            ...
            <div class="rm-card">
                ...
                <div class="rm-kpi-value" style="color:#ef4444;">S/ {{ number_format($datos['totalEgresos'], 2) }}</div>
                <div class="rm-kpi-sub">{{ $datos['egresos']->count() }} transacciones</div>
            </div>

            <div class="rm-card">
                <div class="rm-kpi-icon"
                    style="background:rgba({{ $datos['ahorro'] >= 0 ? '96,165,250' : '251,146,60' }},0.12);">
                    <svg fill="none" viewBox="0 0 24 24" stroke-width="2"
                        stroke="{{ $datos['ahorro'] >= 0 ? '#60a5fa' : '#fb923c' }}">
                        <path stroke-linecap="round" stroke-linejoin="round"
                            d="M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                </div>
                <div class="rm-kpi-label">Ahorro neto</div>
                <div class="rm-kpi-value" style="color:{{ $datos['ahorro'] >= 0 ? '#60a5fa' : '#fb923c' }};">
                    S/ {{ number_format($datos['ahorro'], 2) }}
                </div>
                <div class="rm-kpi-sub">
                    {{ $datos['totalIngresos'] > 0 ? round(($datos['ahorro'] / $datos['totalIngresos']) * 100, 1) : 0 }}%
                    de los ingresos
                </div>
            </div>
        ...
        <div class="rm-grid-53">
            ...
            <div class="rm-card">
                ...
                @forelse($datos['gastosPorCategoria'] as $cat)
                    @php $pct = $datos['totalEgresos'] > 0 ? round(($cat->total / $datos['totalEgresos']) * 100, 1) : 0; @endphp
                    <div class="rm-bar-row">
                        <div class="rm-bar-label">
                            <span class="rm-bar-name">{{ $cat->nombre }}</span>
                            <div class="rm-bar-right">
                                <span class="rm-bar-pct">{{ $pct }}%</span>
                                <span class="rm-bar-amount">S/ {{ number_format($cat->total, 2) }}</span>
                            </div>
                        </div>
            ...
            <div class="rm-card">
                ...
                @foreach ($datos['cuentas'] as $cuenta)
                    <div class="rm-account-row">
                        <div>
                            <div class="rm-account-name">{{ $cuenta->nombre }}</div>
                            <div class="rm-account-type">{{ $cuenta->tipo_cuenta }}</div>
                        </div>
                        <div class="rm-account-saldo" style="{{ $cuenta->saldo_actual < 0 ? 'color:#ef4444;' : '' }}">
                    ...
                    @foreach ($datos['presupuestos'] as $p)
                        ...
                        <div class="rm-budget-card">
                            <div class="rm-budget-header">
                                <div class="rm-budget-name">{{ $p['nombre'] }}</div>
                                <div class="rm-budget-pct" style="color:{{ $bc }};">
                                    {{ $p['porcentaje'] }}%</div>
                            </div>
                            ...
                            <div class="rm-budget-amounts">
                                <span>S/ {{ number_format($p['gasto'], 2) }} gastado</span>
                                <span>límite S/ {{ number_format($p['limite'], 2) }}</span>
                            </div>
                        </div>
                    @endforeach
                </div>
