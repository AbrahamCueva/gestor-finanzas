        ...
        @else
            ...
            <div class="md-recomendado">
                ...
                <div>
                    <div style="font-size:.875rem; font-weight:700; color:#22c55e;">
                        Recomendamos: {{ $d['recomendado'] === 'avalancha' ? '🌊 Método Avalancha' : '⚽ Bola de Nieve' }}
                    </div>
                    <div style="font-size:.72rem; color:var(--muted); margin-top:.15rem;">
                        {{ $d['recomendado'] === 'avalancha'
            ...
            <div class="md-card">
                ...
                <div class="md-metodos">
                    <div class="md-metodo-card {{ $metodo === 'avalancha' ? 'activo-avalancha' : '' }}"
                        ...
                        @if($d['recomendado'] === 'avalancha')
                            <span class="md-metodo-badge" style="background:rgba(34,197,94,.12); color:#22c55e;">✓
                                Recomendado</span>
                        @endif
                    </div>

                    <div class="md-metodo-card {{ $metodo === 'bola_de_nieve' ? 'activo-bola' : '' }}"
                        ...
                        @if($d['recomendado'] === 'bola_de_nieve')
                            <span class="md-metodo-badge" style="background:rgba(34,197,94,.12); color:#22c55e;">✓
                                Recomendado</span>
                        @endif
                    </div>
            ...
            <div class="md-card">
                <div class="md-control">
                    ...
                    <div class="md-control-valor">S/ {{ number_format($pagoMensualExtra, 2) }}/mes</div>
            ...
            <div class="md-comparativa">
                ...
                <div class="md-comp" style="border:1px solid rgba(239,68,68,.2);">
                    ...
                    <div class="md-comp-row">
                        ...
                        <span class="md-comp-value" style="color:var(--text);">{{ $d['avalancha']['meses'] ?? '—' }}
                            meses</span>
                    </div>
                    <div class="md-comp-row">
                        <span class="md-comp-label">Total en intereses</span>
                        <span class="md-comp-value" style="color:var(--red);">S/
                            {{ number_format($d['avalancha']['totalIntereses'] ?? 0, 2) }}</span>
                    </div>
                    <div class="md-comp-row">
                        <span class="md-comp-label">Orden de pago</span>
                        <span class="md-comp-value"
                            style="font-size:.65rem; color:var(--muted);">{{ implode(' → ', array_slice($d['avalancha']['orden'] ?? [], 0, 3)) }}</span>
                    </div>
                    @if(!empty($d['avalancha']['pagadas']))
                        <div style="margin-top:.5rem;">
                            @foreach($d['avalancha']['pagadas'] as $p)
                                <div style="font-size:.65rem; color:#22c55e;">✓ {{ $p['nombre'] }} — mes {{ $p['mes'] }}</div>
                            @endforeach
                        </div>
                    @endif
                </div>
                ...
                <div class="md-comp" style="border:1px solid rgba(96,165,250,.2);">
                    ...
                    <div class="md-comp-row">
                        <span class="md-comp-label">Meses para saldar</span>
                        <span class="md-comp-value" style="color:var(--text);">{{ $d['bolaDeNieve']['meses'] ?? '—' }}
                            meses</span>
                    </div>
                    <div class="md-comp-row">
                        <span class="md-comp-label">Total en intereses</span>
                        <span class="md-comp-value" style="color:var(--red);">S/
                            {{ number_format($d['bolaDeNieve']['totalIntereses'] ?? 0, 2) }}</span>
                    </div>
                    <div class="md-comp-row">
                        <span class="md-comp-label">Orden de pago</span>
                        <span class="md-comp-value"
                            style="font-size:.65rem; color:var(--muted);">{{ implode(' → ', array_slice($d['bolaDeNieve']['orden'] ?? [], 0, 3)) }}</span>
                    </div>
                    @if(!empty($d['bolaDeNieve']['pagadas']))
                        <div style="margin-top:.5rem;">
                            @foreach($d['bolaDeNieve']['pagadas'] as $p)
                                <div style="font-size:.65rem; color:#22c55e;">✓ {{ $p['nombre'] }} — mes {{ $p['mes'] }}</div>
                            @endforeach
                        </div>
                    @endif
                    ...
                    <div>
                        ...
                        <div style="font-size:1.1rem; font-weight:900; color:var(--purple);">
                            ...
                            <span style="font-size:.72rem; font-weight:400; color:var(--muted);">
                                {{ $difMeses > 0 ? 'avalancha es más rápido' : ($difMeses < 0 ? 'bola de nieve es más rápida' : 'igual tiempo') }}
                            </span>
                        </div>
                    </div>
                    <div>
                        ...
                        <div style="font-size:1.1rem; font-weight:900; color:var(--purple);">
                            S/ {{ number_format(abs($difIntereses), 2) }}
                            <span style="font-size:.72rem; font-weight:400; color:var(--muted);">
                                {{ $difIntereses > 0 ? 'ahorras con avalancha' : ($difIntereses < 0 ? 'ahorras con bola de nieve' : 'igual costo') }}
                            </span>
                        </div>
                    </div>
                </div>
            ...
            <div class="md-card">
                ...
                @foreach($d['deudas'] as $deuda)
                    <div class="md-deuda-item">
                        <div>
                            <div class="md-deuda-nombre">{{ $deuda['nombre'] }}</div>
                            <div class="md-deuda-datos">
                                <span>Interés: {{ $deuda['interes'] }}% anual</span>
                                <span>Mínimo: S/ {{ number_format($deuda['minimo'], 2) }}/mes</span>
                            </div>
                        </div>
                        <div class="md-deuda-monto">S/ {{ number_format($deuda['restante'], 2) }}</div>
                    </div>
                @endforeach
