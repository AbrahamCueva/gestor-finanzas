    ...
    <div class="gh">
                ...
                <div class="gh-control">
                    ...
                    <div class="gh-period-btns">
                        @foreach ([1 => '1 mes', 3 => '3 meses', 6 => '6 meses'] as $v => $label)
                            <button class="gh-period-btn {{ $meses == $v ? 'on' : '' }}"
        ...
        <div class="gh-kpis">
            <div class="gh-kpi">
                ...
                <div class="gh-kpi-value" style="color:var(--red);">S/ {{ number_format($d['totalHormiga'], 2) }}</div>
                <div class="gh-kpi-sub">en {{ $d['meses'] }} mes(es)</div>
            </div>
            <div class="gh-kpi">
                <div class="gh-kpi-label">Proyección anual</div>
                <div class="gh-kpi-value" style="color:var(--orange);">S/ {{ number_format($d['proyAnual'], 2) }}</div>
                <div class="gh-kpi-sub">impacto en 12 meses</div>
            </div>
            <div class="gh-kpi">
                <div class="gh-kpi-label">Patrones detectados</div>
                <div class="gh-kpi-value" style="color:var(--gold);">{{ $d['cantidadGrupos'] }}</div>
            ...
            <div class="gh-kpi">
                ...
                <div class="gh-kpi-sub">bajo S/ {{ $montoMaximo }}</div>
            </div>
        </div>
                ...
                @foreach ($d['top5'] as $i => $g)
                    <div class="gh-top-item">
                        ...
                        <div class="gh-top-stats">
                            ...
                            <span class="gh-badge gh-badge-{{ $g['impacto'] }}">{{ $g['impacto'] }}</span>
                        </div>
                    </div>
                @endforeach
            </div>
                    ...
                    <div class="gh-horas-bars">
                        @for ($h = 0; $h <= 23; $h++)
                            ...
                            <div class="gh-hora-bar"
                                style="height:{{ max($perc, 4) }}%;
                                       background:{{ $isMax ? 'var(--gold)' : 'rgba(96,165,250,0.35)' }};"
                                title="{{ $h }}h — S/ {{ number_format($val, 2) }}">
                            </div>
                        @endfor
                    </div>
                    <div
                    ...
                    <table class="gh-table">
                        ...
                        <tbody>
                            @foreach ($d['gastos'] as $g)
                                <tr>
                                    <td style="font-weight:700; color:var(--text);">{{ $g['descripcion'] }}</td>
                                    <td>{{ $g['categoria'] }}</td>
