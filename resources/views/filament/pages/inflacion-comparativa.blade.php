<x-filament-panels::page>
    ...
    <div class="ic-wrap">
        ...
        {{-- Análisis --}}
        <div class="ic-analisis {{ $d['analisis']['estado'] }}">
            <div class="ic-analisis-icon">
                {{ $d['analisis']['estado'] === 'bueno' ? '✅' : '⚠️' }}
            </div>
            <div>
                ...
                <div class="ic-analisis-desc">
                    ...
                    que la inflación ({{ $d['analisis']['pctSuperan'] }}% del tiempo).
                    La inflación acumulada del período fue {{ $d['inflacionAcumulada'] }}%.
                </div>
            </div>
        </div>
        ...
        <div class="ic-card">
            <div class="ic-kpis">
                <div class="ic-kpi">
                    ...
                    <div class="ic-kpi-value" style="color:#fbbf24;">{{ $d['ultimaInflacion'] }}%</div>
                    <div class="ic-kpi-sub">anual (BCRP)</div>
                </div>
                <div class="ic-kpi">
                    <div class="ic-kpi-label">Inflación acumulada</div>
                    <div class="ic-kpi-value" style="color:#f97316;">{{ $d['inflacionAcumulada'] }}%</div>
                    <div class="ic-kpi-sub">en {{ $anios }} año(s)</div>
                </div>
                <div class="ic-kpi">
                    <div class="ic-kpi-label">Meses sobre inflación</div>
                    <div class="ic-kpi-value" style="color:#ef4444;">{{ $d['analisis']['mesesSuperan'] }}</div>
                    <div class="ic-kpi-sub">de {{ count($d['datos']) }} meses</div>
                </div>
                <div class="ic-kpi">
                    <div class="ic-kpi-label">Meses bajo inflación</div>
                    <div class="ic-kpi-value" style="color:#22c55e;">{{ $d['analisis']['mesesBajo'] }}</div>
                    <div class="ic-kpi-sub">de {{ count($d['datos']) }} meses</div>
                </div>
            </div>
        </div>
        ...
        <div class="ic-grid-2">
            <div class="ic-poder-card" style="border-color:{{ $d['diferenciaPoder'] > 0 ? '#ef4444' : '#22c55e' }};">
                ...
                <div class="ic-poder-desc">
                    ...
                    @else
                        Tus gastos están S/ {{ number_format(abs($d['diferenciaPoder']), 2) }} por debajo de lo que
            ...
            <div class="ic-poder-card" style="border-color:#60a5fa;">
                ...
                <div style="display:flex; flex-direction:column; gap:0.375rem; margin-top:0.375rem;">
                    ...
                    <div
                        ...
                        <span
                            style="font-weight:700; color:{{ $d['ultimoGasto'] > $d['gastoAjustado'] ? '#ef4444' : '#22c55e' }};">
                            S/ {{ number_format($d['ultimoGasto'], 2) }}
                        </span>
                    </div>
                </div>
            </div>
                        ...
                        @foreach (array_reverse($d['datos']) as $fila)
                            <tr>
                                ...
                                <td style="text-align:right; font-weight:600;">
                                    S/ {{ number_format($fila['gasto'], 2) }}
                                </td>
                                <td
                                    style="text-align:right; font-weight:700; color:{{ $fila['variacionGasto'] > 0 ? '#ef4444' : ($fila['variacionGasto'] < 0 ? '#22c55e' : '#6b7280') }};">
                                    {{ $fila['variacionGasto'] > 0 ? '+' : '' }}{{ $fila['variacionGasto'] }}%
                                </td>
                                <td style="text-align:right; color:#fbbf24; font-weight:600;">
                                    {{ $fila['inflacionMensual'] }}%
                                </td>
