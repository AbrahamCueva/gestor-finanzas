<x-filament-panels::page>
    ...
    <div class="ra-space">
        ...
        <div style="display:flex; align-items:center; gap:0.75rem;">
            ...
            <span style="font-size:0.8rem; color:var(--w-muted);">Resumen del año {{ $datos['anio'] }}</span>
        </div>
        ...
        <div class="ra-grid-3">
            <div class="ra-kpi">
                ...
                <div class="ra-kpi-value" style="color:#22c55e;">S/ {{ number_format($datos['totalIngresos'], 2) }}
                </div>
                <div class="ra-kpi-sub">Promedio S/ {{ number_format($datos['totalIngresos'] / 12, 2) }}/mes</div>
            </div>
            ...
            <div class="ra-kpi">
                ...
                <div class="ra-kpi-value" style="color:#ef4444;">S/ {{ number_format($datos['totalEgresos'], 2) }}</div>
                <div class="ra-kpi-sub">Promedio S/ {{ number_format($datos['totalEgresos'] / 12, 2) }}/mes</div>
            </div>

            <div class="ra-kpi">
                <div class="ra-kpi-icon"
                    style="background:rgba({{ $datos['totalAhorro'] >= 0 ? '96,165,250' : '251,146,60' }},0.12);">
                    <svg fill="none" viewBox="0 0 24 24" stroke-width="2"
                        stroke="{{ $datos['totalAhorro'] >= 0 ? '#60a5fa' : '#fb923c' }}">
                        <path stroke-linecap="round" stroke-linejoin="round"
                            d="M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                </div>
                <div class="ra-kpi-label">Ahorro anual</div>
                <div class="ra-kpi-value" style="color:{{ $datos['totalAhorro'] >= 0 ? '#60a5fa' : '#fb923c' }};">
                    S/ {{ number_format($datos['totalAhorro'], 2) }}
                </div>
                <div class="ra-kpi-sub">
                    {{ $datos['totalIngresos'] > 0 ? round(($datos['totalAhorro'] / $datos['totalIngresos']) * 100, 1) : 0 }}%
                    de los ingresos
                </div>
            </div>
        </div>
        ...
        <div class="ra-highlight">
            <div class="ra-hl-card">
                ...
                <div class="ra-hl-mes">{{ $datos['mejorMes']['mes'] }}</div>
                <div class="ra-hl-value">S/ {{ number_format($datos['mejorMes']['ahorro'], 2) }} ahorrado</div>
            </div>
            <div class="ra-hl-card">
                <div class="ra-hl-label" style="color:#ef4444;">📉 Mes más ajustado</div>
                <div class="ra-hl-mes">{{ $datos['peorMes']['mes'] }}</div>
                <div class="ra-hl-value">S/ {{ number_format($datos['peorMes']['ahorro'], 2) }} ahorrado</div>
            </div>
        </div>
        ...
        <div class="ra-card">
            ...
            <table class="ra-table">
                ...
                <tbody>
                    @foreach ($datos['meses'] as $m)
                        <tr>
                            <td>{{ $m['mes'] }}</td>
                            <td style="color:#22c55e; font-weight:600;">S/ {{ number_format($m['ingresos'], 2) }}</td>
                            <td style="color:#ef4444; font-weight:600;">S/ {{ number_format($m['egresos'], 2) }}</td>
                            <td style="color:{{ $m['ahorro'] >= 0 ? '#60a5fa' : '#fb923c' }}; font-weight:700;">
                    ...
                    <tr class="ra-total">
                        ...
                        <td style="color:{{ $datos['totalAhorro'] >= 0 ? '#60a5fa' : '#fb923c' }};">S/
                            {{ number_format($datos['totalAhorro'], 2) }}</td>
                    </tr>
                </tbody>
            </table>
        </div>
        ...
        <div class="ra-grid-2">
            ...
            <div class="ra-card">
                ...
                @php $maxCat = $datos['topCategorias']->max('total') ?: 1; @endphp
                @forelse($datos['topCategorias'] as $cat)
                    <div class="ra-bar-row">
                        <div class="ra-bar-label">
                            <span class="ra-bar-name">{{ $cat->nombre }}</span>
                            <span class="ra-bar-amount">S/ {{ number_format($cat->total, 2) }}</span>
                        </div>
                        <div class="ra-bar-track">
                            <div class="ra-bar-fill" style="width:{{ round(($cat->total / $maxCat) * 100) }}%;"></div>
                        </div>
                    </div>
                @empty
            ...
            <div class="ra-card">
                ...
                @foreach ($datos['cuentas'] as $cuenta)
                    <div class="ra-cuenta-row">
                        <div>
                            <div class="ra-cuenta-name">{{ $cuenta->nombre }}</div>
                            <div class="ra-cuenta-type">{{ $cuenta->tipo_cuenta }}</div>
                        </div>
                        <div class="ra-cuenta-saldo" style="{{ $cuenta->saldo_actual < 0 ? 'color:#ef4444;' : '' }}">
                    ...
                    @foreach ($datos['presupuestos'] as $p)
                        ...
                        <div class="ra-budget-card">
                            ...
                            <div class="ra-budget-amounts">
                                <span>S/ {{ number_format($p['gasto'], 2) }}</span>
                                <span>/ S/ {{ number_format($p['limite'], 2) }}</span>
                            </div>
                        </div>
                    @endforeach
                </div>
