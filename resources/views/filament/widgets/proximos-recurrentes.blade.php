        ...
        @else
            <div class="pr-grid">
                @foreach ($movimientos as $m)
                    ...
                    @endphp
                    <div class="pr-card">
                        <div class="pr-card-accent" style="background:{{ $color }};"></div>

                        <div class="pr-card-dot" style="background:{{ $bgColor }};">
                            <svg fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="{{ $color }}">
                                @if ($esIngreso)
                        ...
                        <div class="pr-card-body">
                            <div class="pr-card-name">{{ $m['nombre'] }}</div>
                            <div class="pr-card-meta">
                                <span>{{ $m['cuenta'] }}</span>
                                <span class="pr-card-sep">·</span>
                                <span>{{ ucfirst($m['frecuencia']) }}</span>
                            </div>
                        </div>

                        <div class="pr-card-right">
                            <div class="pr-card-amount" style="color:{{ $color }};">
                                {{ $esIngreso ? '+' : '-' }}S/ {{ number_format($m['monto'], 2) }}
                            </div>
                            <div class="pr-card-when"
                                style="background:{{ $whenBg }}; color:{{ $whenColor }};">
                                {{ $whenLabel }}
                            </div>
                        </div>
                    </div>
